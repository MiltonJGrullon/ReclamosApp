...
namespace Reclamos
{
    public partial class FrmTiposRedesSociales : Form
    {
        public FrmTiposRedesSociales()
        {
            InitializeComponent();
            limpiar();
        ...
        {
            dtdata = Ctool.ExcSqlDT("select id, nombre from Gen.Tipos_RedesSociales where idcompania = " + Ctool.cia);
            if (Ctool.OcError)
            {
                return;
            }
        ...
        {
            DataTable dt = Ctool.ExcSqlDT("Select * from Gen.Tipos_RedesSociales where idcompania = " + Ctool.cia + " and id = " + txtcod.Text.Trim());
            if (dt.Rows.Count > 0)
            {
        ...
        private void btnsalvar_Click(object sender, EventArgs e)
        {
            ...
            Ctool.ExcSql($"exec Gen.proc_tredessociales @idcompania = {Ctool.cia} ,@id = {vcod},@nombre = '{vdes}' ");
            if (Ctool.OcError)
            {
        ...
        private void btnborrar_Click(object sender, EventArgs e)
        {
            ...
            Ctool.ExcSql($"delete from Gen.Tipos_RedesSociales  where idcompania = {Ctool.cia} and id = {vcod}");
            if (Ctool.OcError)
            {
                return;
            }
